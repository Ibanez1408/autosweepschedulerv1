...
export class ProfileComponent implements OnInit {
    ...
    private getLocations(): void {
        this.as.getLocations().subscribe((data: any) => {
            if (data.success) {
               ...
               console.log(this.uniqueTollway);
            }
        });
    }
    ...
    filterByTollway(item: string) {
        const arr = this.locations.filter(x => x.Channel === item);
        this.uniqueChannel = [... new Set(arr.map(item => item.Channel))];
        console.log(this.uniqueChannel);
    }
    //#endregion
}
