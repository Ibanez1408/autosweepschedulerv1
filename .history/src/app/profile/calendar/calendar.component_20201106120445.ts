  ...
  handleDateClick(arg) {
    ...
    if (x !== undefined && x.isAvailable ) {
      ...
      this.alerts.push({
        ...
        type: 'warning',
        strong: 'Sorry!',
        message: `This date, ${arg.dateStr}, is not available.`,
        icon: ''
      });
    }  else {
      this.showSuccess = false;
    ...
    this.as.queryAvailability(this.selectedDate, this.selectedLocation.Location).subscribe((data: any) => {
      if (data.success) {
        const schedule: Schedule = {
          ...
          PlateNo: xForm.plateNumber.replace(/\s/g, ''),
        ...
        this.as.sendSchedule(schedule).subscribe((data1: any) => {
          if (data1.success) {
            ...
            this.alerts.push({
              ...
              type: 'success',
              strong: 'Ok!',
              message: `${data1.message}. Please remember your reference number.`,
              icon: 'ni ni-like-2'
            });
            this.form.reset();
            this.sentStatusCode = 1;
            ...
            this.alerts.push({
              ...
              type: 'warning',
              strong: 'Notice',
              message: `${data1.message}. Your submission has failed`,
              icon: ''
            });
          }
        });
