...
export class ProfileComponent implements OnInit {
    ...
    public title = 'Choose Installation Site';
    public step = 1;
    ...
    filterByChannel(item: string) {
        ...
        this.step = 3;
        const arr = this.locations.filter(x1 => x1.Channel === item && x1.Tollway === this.selectedTollway);
        let x: Locations[] = null;
        this.uniqueLocation = [... new Set(arr.map(i => i.Location))];
        this.uniqueLocation.forEach(element => {
            x = this.locations.filter(x1 => x1.Location === element);
            arr.concat(x);
        });
    ...
    selectLocation(item: Locations): void {
        ...
        this.as.getCalendarAvailability(this.selectedLocation.Location).subscribe((data: any) => {
            if (data.success) {
                ...
                this.as.setSelectedStation(this.selectedLocation);
            }
        });
    }

    goBack(stepNumber: number) {
        stepNumber--;
        this.step = stepNumber;
        switch(this.step) {
            case 1:
                this.title = 'Choose Installation Site';
                break;
            case 2:
                this.title = 'Choose Channel';
                break;
